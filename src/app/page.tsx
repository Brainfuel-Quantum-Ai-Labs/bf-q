      ...
      <section className="py-24 relative">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-end justify-between mb-12">
            ...
            <Link
              href="/research"
              className="hidden sm:inline-flex items-center gap-1 text-sm text-quantum-400 hover:text-quantum-300 font-medium"
            >
            ...
            {recentPosts.map((post) => (
              <Link key={post.slug} href={`/research/${post.slug}`} className="group">
                <div className="p-6 rounded-xl border border-white/10 bg-card hover:border-quantum-700/50 transition-colors h-full flex flex-col">
                  ...
                  <h3 className="font-semibold mb-3 group-hover:text-quantum-300 transition-colors flex-grow leading-snug">
                    {post.title}
                  </h3>
          ...
          <div className="mt-6 sm:hidden text-center">
            <Link href="/research" className="text-sm text-quantum-400 hover:text-quantum-300 font-medium">
              View all research →
            </Link>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {activeProjects.map((project) => (
              <div
                key={project.slug}
                className="p-6 rounded-xl border border-green-800/40 bg-green-950/10 hover:border-green-600/60 transition-colors"
              >
                ...
                <div className="flex flex-wrap gap-1.5">
                  {project.tags.slice(0, 3).map((tag) => (
                    <span
                      key={tag}
                      className="px-2 py-0.5 text-xs rounded-full border border-green-900/50 bg-green-950/30 text-green-400 font-medium"
                    >
                      {tag}
                    </span>
                  ))}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
            {[
              ...
              <Link key={title} href={href} className="group">
                <div className={`p-6 rounded-xl border ${border} bg-card transition-colors`}>
